...
class CitySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'name', 'description', 'cover_image',
                  'created_at', 'updated_at', 'avg_rating', 'rating_count', 'attractions']
...
class AttractionMinimalSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Attraction
        fields = ['id', 'name', 'city_name', 'cover_image', 'avg_rating', 'rating_count']
...
class AttractionDetailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'name', 'city', 'city_name', 'description',
                  'official_url', 'cover_image', 'avg_rating', 'rating_count', 'is_bookmarked']
        read_only_fields = ['id']
...
class CityRatingSerializer(serializers.ModelSerializer):
    ...
    Auto-associates current user (prevents manual user ID tampering)
    """
    # Hide user field (auto-populated from request)
    user = serializers.HiddenField(default=serializers.CurrentUserDefault())
    # Human-readable city name (optional, for response)
    city_name = serializers.CharField(source='city.name', read_only=True)
    ...
    # Custom validation: Ensure rating is between 1-5 stars
